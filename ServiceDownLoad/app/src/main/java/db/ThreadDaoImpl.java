...
public class ThreadDaoImpl implements ThreadDao {
    ...
    public ThreadDaoImpl(Context contex) {
        mHelper = new DBHelper(contex);
    ...
    public boolean isExits(String url, int thread_id) {
        SQLiteDatabase db = mHelper.getReadableDatabase();
        Cursor c = db.rawQuery("select * from thread_info where url=? and thread_id =?", new String[]{url, thread_id + ""});
        boolean exits = c.moveToNext();
        c.close();
        db.close();
        return exits;
    }
